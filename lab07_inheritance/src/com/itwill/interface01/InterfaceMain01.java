 ...
 *   class T implements A, B, C {} => 여기서 A, B, C는 클래스
 * 인터페이스는 상위 인터페이스를 상속(확장)할 수 있음. 인터페이스는 다중 상속이 가능.
 *   interface I extends A, B, C {} => 여기서 A, B, C는 인터페이스.
 */
...
//		MySqlDatabase db = new MySqlDatabase();
		...
		Database db = new MySqlDatabase(); // => 다형성을 이용해서 재사용성 높이기.
//		Database db = new OracleDatabase(); // => 한 줄의 오른쪽 항만 바꾸면 가져오는 클래스를 바꿀 수 있음.
		// Database.DB_VERSION // 인터페이스에서 int DB_VERSION으로 선언했지만 Public static final로 선언됨.
		
		int result = db.insert();
		System.out.printf("insert result = %d\n", result);
		
		result = db.select();
		System.out.printf("select result = %d\n", result);
