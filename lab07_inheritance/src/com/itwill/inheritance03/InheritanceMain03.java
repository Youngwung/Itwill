...
public class InheritanceMain03 {
	...
	public static void main(String[] args) {
		...
		// SubType var = new SuperType(); => 컴파일 에러
//		HybridCar car4 = new Car(100, 100);
		car3.drive(); // ->다형성으로 선언되어 있더라도, override 되어있는 메서드가 호출됨! => Car클래스의 메서드를 호출시도하지만 HybridCar클래스의 오버라이드되어있는 drive를 호출함.
//		car3.charge(100); => -> casting을 하지 않으면 SubType의(실제 객체의) 모든 메서드를 사용할 수는 없음.
		((HybridCar) car3).charge(100); //-> casting을 하면, SubType의 모든 메서드를 사용할 수 있음.
		
		// 다형성의 장점: 코드의 재사용성(re-usability)을 높여줌.
		// (ex) 배열 선언, 파라미터 선언 등에서 
		// 		상위 타입으로 선언한 변수에 하위 타입 객체들을 할당할 수 있기 때문에 
		// 		코드 재사용성이 높아짐.
		Car[] cars = new Car[3];
		cars[0] = car1; // new car(...)
		...
		InheritanceMain03 te = new InheritanceMain03();
	...
	public void test(Car car) {
		...
		}// Car객체와 Hybrid객체가 하는 일이 다를 때 객체를 구분하기 위한 메서드 if문 사용법.
	}
