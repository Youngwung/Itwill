 ...
 * 
 * 내부 클래스를 선언할 수 있는 위치: 
 * 1. 멤버 내부 클래스(필드 위치에 선언, 외부 클래스 안쪽 && 메인메서드 바깥쪽)
 * 		(1) 인스턴스 내부 클래스: static이 아니느 멤버 내부 클래스.
 * 			외부 클래스의 객체를 먼저 생성한 후에, 
 * 			그 참조변수로 참조해서 객체를 생성하고 사용할 수 있는 멤버 내부 클래스.
 * 		(2) 정적(static) 내부 클래스: static으로 선언한 멤버 내부 클래스.
 * 			외부 클래스의 객체 생성 여부와 상관 없이 사용할 수 있는 멤버 내부 클래스.
 * 			=> 중첩 클래스(nexted class).
 * 			멤버 내부 인터페이스는 static이어야만 함. static은 생략.
 * 			
 * 2. 지역 내부 클래스(local class)(지역변수 위치에 선언, 메인메서드 안쪽)
 ...
 * 
 * 상속과 다른 점: 상속은 상속받은 클래스의 참조변수는 상속한 필드와 메서드를 사용 가능하지만
 *  내부클래스는 내부클래스 안에서만 필드와 메서드가 사용 가능하다.
 *  		내부클래스 안에서는 필드와 메서드 사용이 자유롭지만 
 *  		메인에서 호출한 내부클래스의 인스턴스로는 외부클래스에서 선언한 필드와 메서드를 볼 수 없다.
 */
...
public class InnerMain {
	...
	public static void main(String[] args) {
		...
		// import문 작성 시 나름 간단하게 작성가능
		// 내부 클래스 이름을 import 한 경우. 참조변수 생성 할 때 외부클래스 이름 생략 가능.
		InnerCls inner2 = outer1.new InnerCls(200);
		inner2.info();
		
		// 이런것도 됨
		InnerCls inner3 = new OuterCls(1, 20, "점심시간").new InnerCls(123);
		inner3.info();
		
		// EnclosingCls의 static field
		System.out.println(EnclosingCls.var);
		
		// EnclosingCls의 static method
		EnclosingCls.test();
		
		// EnclosingCls 타입의 객체 생성
		...
		// 간단하게 작성 가능.
		NestedCls nested2 = new NestedCls(5);
		nested2.info();
