 ...
 * InputStream
 * |__ FileInputStream, BufferedInputStream, ObjectInputStream, ...  => byte Stream
 * 
 * OutputStream
 * |__ FlieOutputStream, BufferedOutputStream, ObjectOutputStream, ...  => byte Stream
 * 
 * Reader/Writer: 문자 스트림(character stream). 읽기/쓰기의 기본 단위는 "문자(character)".
 * 		텍스트 파일(txt, csv, ... => 메모장으로 열리는 확장자)에서만 사용.
 * Reader
...
public class FileMain07 {
	...
	public static void main(String[] args) throws FileNotFoundException, UnsupportedEncodingException, Exception {
		...
		FileInputStream in = new FileInputStream(ansiFile); //원래 try-catch로 묶어야하지만 문법에 집중하기 위해 던져버림
		// 2. 인코딩을 설정한 InputStreamReader(문자 스트림)객체를 생성.
		InputStreamReader isr = new InputStreamReader(in, "EUC-KR"); // 인코더 설정.ANSI를 읽을 수 있는 인코더를 설정.
		...
		// 자바 11 버전부터 FileReader, FileWriter 클래스에서 인코딩을 설정할 수 있는 
		// 생성자를 제공하기 시작.
		// 1. FileReader 객체를 생성할 때 인코딩 방식을 설정.
		FileReader fr = new FileReader(utf8File, Charset.forName("UTF-8")); // Charset = 클래스. utf-8로 인코딩을 설정함.
		// 2. 읽기 속도를 빠르게 하기 위해서 버퍼 사용.
		BufferedReader br = new BufferedReader(fr);
		
//		while(true) { // 한 글자 씩 읽기
//			int read = br.read()
//			if (read == -1) {
//				break;
//			}
