...
public class ScannerTest {
	...
	public static void main(String[] args) {
		...
		// 입력값(토큰)을 읽어서 int 타입 정수를 반환. 읽은 값까지만 입력 버퍼를 지운다.
		// nextDouble(): 입력 버퍼에서 공백문자로 구분된 입력값을 읽어서 double 타입 실수를 반환.
		// next(): 입력 버퍼에서 공백문자로 구분된 입력값을 읽어서 문자열(String)을 반환.
		// 위에있는 코드들은 입력버퍼를 초기화 시켜주지 않는다.
		// 입력버퍼가 남아있다면 사용자의 입력을 기다리지 않고 입력버퍼에 남아있는 걸 바로 출력한다.
		// nextLine(): 입력 버퍼에서 엔터를 만날 때까지 모든 값을 읽어서 문자열을 반환.
		// nextLine()을 호출(실행)하면 입력 버퍼의 모든 내용이 지워짐.***
