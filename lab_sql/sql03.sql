/*
    ...
    테이블에서 데이터 검색: 
    (1) projection: 테이블에서 원하는 컬럼을 선택.
    (2) selection: 테이블에서 조건을 만족하는 행(들)을 검색.
    
    문법: select 컬럽, ... from 테이블 where 조건식 (order by 컬럼, ...); => 정렬식 order by는 항상 '마지막에!'
    조건식에서 사용되는 연산자들:
    (1) 비교 연산자:  =, !=, >, >=, <, <=, is null, is not null, ...
    (2) 논리 연산자: and, or, not, 
*/ 

-- 직원 테이블에서 10번 부서에서 근무하는 직원들의 부서번호, 사번, 이름
-- 사번 오름차순
...
-- 암묵적(자동) 타입 변환:
-- 오라클은 hiredate 컬럼(date 타입, 날짜타입)과 문자열 '1987/01/01'의 크기를 비교할 때
-- 날짜 타입(hiredate)을 문자열로 변환한 후 문자열의 크기 비교를 수행함.
...
-- (1) %: 글자 수 제한 없음. %이후에 몇 글자든 상관없음.
-- (2) _(underscore, 밑줄): 밑줄이 있는 자리에 어떤 문자가 있어도 됨. 
-- ex) like '_A' => 두번째 글자가 A이고 총 글자 수가 2자리인 문자열만 true.
-- ex2) like '--A%' => 세번째 글자가 A인 모든 문자열이 ture. (글자 수 상관X)
select * 
from emp
